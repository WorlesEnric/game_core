...
def main() -> int:
    ...
    if check:
        ...
        if committed != text:
            ...
            return 1
        print("%s reproduces %s byte for byte (%d bytes, hash %s)"
              % (description_path.name, output_path.name, len(text.encode("utf-8")), file_hash))
        return 0

    output_path.write_text(text, encoding="utf-8")
    print("wrote %s (%d bytes, CatalogFileHash %s, %d groups, %d schemas)"
          % (output_path, len(text.encode("utf-8")), file_hash, len(model["groups"]), len(model["schemas"])))
    return 0
...
def run_self_check() -> int:
    """Regenerate every committed catalog from its description and compare byte for byte."""
    failures = 0
    for description_relative, catalog_relative in COMMITTED_CATALOGS:
        ...
        if not description_path.exists() or not catalog_path.exists():
            print("MISSING: %s or %s" % (description_relative, catalog_relative))
            failures += 1
            continue
        ...
        if text != committed:
            ...
            failures += 1
            continue
        print("ok: %s -> %s (%d bytes, %d groups, %d schemas, CatalogFileHash %s)"
              % (description_relative, catalog_relative, len(text.encode("utf-8")),
                 len(model["groups"]), len(model["schemas"]), file_hash))
    if failures:
        print("%d committed catalog(s) drifted from their description" % failures)
        return 1
    print("every committed catalog reproduces byte for byte from its committed description")
    return 0
