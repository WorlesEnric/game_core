#!/usr/bin/env bash
# Wave 2 integration gate (W2-GATE).
#
# Runs, in order:
#   1. the whole plain-dotnet solution: build and test (every Unity-free assembly, including GC-006's derivation,
#      GC-007's ownership/messages, GC-008's planner and GC-009's schedule compiler),
#   2. Unity package resolution on the qualification project (refreshes packages-lock.json),
#   3. the Unity EditMode suite of every testable package plus the W1 and W2 gate EditMode assemblies,
...
#      dispatch, the W1 gate and the W2 gate,
#   7. the documentation validator.
#
# The W2 gate is an integrated gate: it is never claimed from the dotnet half alone, and it never substitutes a
# seam fixture for a real W2 module.
#
...
#   ARTIFACTS     artifact directory (default: <repo>/artifacts/w2-gate)
...
if [[ -z "${UNITY:-}" ]]; then
  ...
  echo "The W2 gate is an integrated gate: it is never claimed from the dotnet half alone." >&2
  exit 2
fi
if [[ ! -x "${UNITY}" ]]; then
...
# 1. The whole plain-dotnet solution: the Unity-free half of every Wave 2 module.
...
# 2. Package resolution: every Wave 2 package must resolve before any test runs.
run_step unity-resolve "${UNITY}" \
  -batchmode -nographics -quit \
  -projectPath "${UNITY_PROJECT}" \
  -logFile "${ARTIFACTS}/unity/resolve.log"

# 3. EditMode tests: every testable package plus the two gate assemblies.
...
echo "== W2 integration gate PASSED =="
