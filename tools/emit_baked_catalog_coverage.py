...
def validate(document):
    ...
    for i, target in enumerate(targets):
        ...
        by_role[role] = {
            "role": role,
            "recipeDefinitionStableName": stable_name(
                target.get("recipeDefinitionStableName"), where + ".recipeDefinitionStableName"),
...
def emit(authoring_text, roles):
    ...
    out.append("//\n")
    out.append("// Baked artifact of " + AUTHORING_PATH + " (04 section 6: baking is an Editor workflow, and the player\n")
    ...
    out.append("\n")
    out.append("        /// <summary>Stable definition name of the baked runner recipe (P-004).</summary>\n")
    out.append("        public const string RunnerRecipeDefinitionStableName = \""
               + runner["recipeDefinitionStableName"] + "\";\n")
    out.append("\n")
