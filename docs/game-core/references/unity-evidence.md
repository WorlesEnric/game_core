...
**Baseline replacement (GC-001).** The qualification profile was subsequently replaced: the build host is **Linux x86_64 (Ubuntu 24.04)** and the first standalone target is **StandaloneLinux64 x86_64, IL2CPP, High managed stripping**, built and run headless with `-batchmode -nographics`. The Editor and runtime package pins above are unchanged; the native toolchain is now the Linux IL2CPP toolchain and sysroot from `com.unity.toolchain.linux-x86_64` (2.0.11 for this Editor line) instead of Xcode 16.4 and the macOS 15.5 SDK. The reason is the selected build host, not a package finding. The first real Unity resolve selected the Editor-bundled **Test Framework 1.6.0** instead of the requested 1.4.6; the manifest now pins 1.6.0 and the generated lock records that result. The registry research below remains historical evidence, not the resolved graph. The macOS ARM64 profile and the Apple toolchain facts below remain researched but **unqualified**: they need their own build and player run before any macOS claim. [Unity integration](../04-unity-integration.md) section 1 records the replacement profile as the normative baseline.
