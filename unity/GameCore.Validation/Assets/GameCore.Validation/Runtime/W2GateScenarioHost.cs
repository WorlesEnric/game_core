...
{
    /// <summary>
    /// W2 integration gate host of the qualification project: it supplies the two catalogs the gate scenario is run
    ...
    ///
    /// Both runs share one implementation (<c>W2GateScenario</c>), so the EditMode test and the player probe exercise
    /// identical scenarios. Each run tears its own world down and leaves the owned-world registry as it found it.
    /// </summary>
    public static class W2GateScenarioHost
    {
        ...
        public static W2GateScenarioResult RunGeneratedCatalog()
        {
            ...
            // Both providers mount through the same precompiled factory the committed catalog registers; the plugin
            // types differ because one catalog declaration resolves one manifest (P-009, 04 section 8).
            ...
            return W2GateScenario.Run(
                build.Catalog,
                declarations,
                ProbeKeys.AbsentFixturePluginKey,
                W2GateKeys.PluginTypeId(3UL),
                emittedFingerprint);
        }

        /// <summary>Runs the gate against the hand-written generated-style catalog in the fixture package.</summary>
        public static W2GateScenarioResult RunFixtureCatalog() => W2GateScenario.RunFixtureCatalog();
        ...
        /// </summary>
        public static IReadOnlyList<W2GateStep> RunBoth(out W2GateFacts generatedFacts, out W2GateFacts fixtureFacts)
        {
            W2GateScenarioResult generated = RunGeneratedCatalog();
            generatedFacts = generated.Facts;

            W2GateScenarioResult fixture = RunFixtureCatalog();
            fixtureFacts = fixture.Facts;

            var combined = new List<W2GateStep>(generated.Steps.Count + fixture.Steps.Count);
            ...
            {
                W2GateStep step = fixture.Steps[i];
                combined.Add(new W2GateStep(FixtureRunPrefix + step.Name, step.Passed, step.Detail));
            }
