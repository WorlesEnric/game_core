...
namespace GameCore.Validation.ProbeHost
{
    ...
    /// Wave 4 gate probes.
    ///
    /// The runner (`Gc019Scenario` over `Gc019NarrativeHost` and `Gc019CardsHost`) is shared with the Unity EditMode
    ...
    public static class ProbeGc019
    {
        ...
        public static void Run(ProbeReport report)
        {
            ...
            RunFamily(report, Gc019NarrativeHost.Label, NarrativeDigest);
            RunFamily(report, Gc019CardsHost.Label, CardsDigest);
        }
        ...
        private static void RunFamily(ProbeReport report, string label, string expectedDigest)
        {
            try
            {
                ...
                if (string.Equals(label, Gc019NarrativeHost.Label, StringComparison.Ordinal))
                {
                    combined = Gc019NarrativeHost.RunBothGc019(out generated, out fixture);
                }
                else
                {
                    combined = Gc019CardsHost.RunBothGc019(out generated, out fixture);
                }
