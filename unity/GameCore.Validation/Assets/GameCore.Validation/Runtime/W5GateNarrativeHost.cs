// GameCore.Validation.ProbeHost - the Wave 5 gate's narrative family adapter.
//
// `W5GateFamily.cs` defines what the gate requires from one genre (the checkpoint contract and the adapter contract
// at once); `Gc013NarrativeHost.NarrativeFamily` already implements both halves in its other partial parts. This part
// adds exactly two things and nothing else:
//
//   * the declaration that the narrative family satisfies `IW5GateFamily`, so a family that lost one of the two
//     contracts fails to compile rather than at run time;
//   * the two catalog entry points the Wave 5 probe and the EditMode suite call, mirroring
//     `RunGeneratedCatalogGc018`/`RunGeneratedCatalogGc019` so the gate runs the *same* narrative world the earlier
//     gates run - the committed generated catalog and the hand-written generated-style catalog, nothing new.
#nullable enable
using System;
...
namespace GameCore.Validation.ProbeHost
{
    ...
    {
        /// <summary>The Wave 5 gate half of the narrative family: it satisfies both gate contracts (P-001).</summary>
        public sealed partial class NarrativeFamily : IW5GateFamily
        {
        }

        /// <summary>Runs the Wave 5 gate against the committed generated catalog (GC-003 compiler output).</summary>
        public static W5GateScenarioResult RunGeneratedCatalogW5Gate()
        {
            ...
            return W5GateScenario.Run(new NarrativeFamily(
                catalog,
                Declarations(ProbeCatalog.FixturePluginKey, W1GateKeys.CatalogSchema),
                ProbeCatalog.CatalogFingerprint));
        }

        /// <summary>Runs the Wave 5 gate against the hand-written generated-style catalog in the fixture package.</summary>
