...
namespace GameCore.Validation.ProbeHost
{
    ...
    public static class Gc018Scenario
    {
        ...
        private sealed class Executor
        {
            ...
            /// because a capture without every required serializer must be refused rather than write a partial
            /// document (05 s6, P-053, P-054).
            /// </summary>
            private bool TryBuildCodecs(out string detail)
            {
                bool built = Gc018CheckpointCodecs.TryBuild(out CheckpointSerializerBindings? builtBindings,
                    out CheckpointCodecSet? builtCodecs, out detail);
                bindings = builtBindings;
                codecs = builtCodecs;
                if (built)
                {
                    codecEvidence = detail;
                }
